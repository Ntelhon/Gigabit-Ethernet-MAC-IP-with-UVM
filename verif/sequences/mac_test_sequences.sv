...
//==============================================================================
class mac_init_seq extends mac_virtual_sequence_base;
...
//==============================================================================
// Sequence: mac_tx_basic_seq
// Description: RX path test sequence (legacy name for compatibility)
//
// NOTE: Despite the name, this sequence tests the MAC's RECEIVE path by
//       injecting frames via GMII RX interface. This is identical to
//       mac_rx_basic_seq. The name is preserved for backward compatibility.
//
// DEPRECATED: Use mac_rx_basic_seq for new tests.
//==============================================================================
class mac_tx_basic_seq extends mac_virtual_sequence_base;

    `uvm_object_utils(mac_tx_basic_seq)
    
    int frame_size = 64;
    bit [47:0] dst_mac = 48'h001122334455;  // Send to MAC's own address
    bit [47:0] src_mac = 48'hAABBCCDDEEFF;  // Different source address

    function new(string name = "mac_tx_basic_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("SEQ", $sformatf("[DEPRECATED] mac_tx_basic_seq: Testing RX path, frame_size=%0d", frame_size), UVM_MEDIUM)
        
        // Read initial RX frame count (not TX!)
        ...
        frame.has_fcs = 1;  // Include FCS for RX frames
        frame.inject_fcs_error = 0;
        
        // Send frame TO the MAC (this will increment RX count)
        send_frame(frame);
        ...
        // Verify RX frame count incremented
...
//==============================================================================
class mac_rx_basic_seq extends mac_virtual_sequence_base;
    ...
    virtual task body();
        ...
        // Send frame (driver injects to RX side)
        send_frame(frame);
...
//==============================================================================
class mac_crc_error_seq extends mac_virtual_sequence_base;
...
//==============================================================================
class mac_runt_frame_seq extends mac_virtual_sequence_base;
...
//==============================================================================
class mac_ifg_test_seq extends mac_virtual_sequence_base;
...
//==============================================================================
class mac_reset_during_traffic_seq extends mac_virtual_sequence_base;
