...
class dma_base_test extends mac_base_test;
    ...
    virtual task configure_dma(
        ...
        // Reset DMA
        write_reg(32'h0200, 32'h0000_0300);  // TX_RST | RX_RST
        #1us;
        write_reg(32'h0200, 32'h0000_0000);
        #1us;
        
        // Configure TX ring
        if (enable_tx) begin
            write_reg(32'h0210, tx_desc_base[31:0]);   // TX_DESC_LO
            write_reg(32'h0214, tx_desc_base[63:32]);  // TX_DESC_HI
            write_reg(32'h0218, desc_count);           // TX_DESC_SIZE
            write_reg(32'h021C, 0);                    // TX_HEAD
            write_reg(32'h0220, 0);                    // TX_TAIL
        end
        
        // Configure RX ring
        if (enable_rx) begin
            write_reg(32'h0240, rx_desc_base[31:0]);   // RX_DESC_LO
            write_reg(32'h0244, rx_desc_base[63:32]);  // RX_DESC_HI
            write_reg(32'h0248, desc_count);           // RX_DESC_SIZE
            write_reg(32'h024C, 0);                    // RX_HEAD
            write_reg(32'h0250, 0);                    // RX_TAIL
        end
        
        // Enable interrupts
        if (enable_irq) begin
            write_reg(32'h0208, 32'h0000_0303);  // TX/RX done + error
        end
    ...
    virtual task setup_rx_buffers(int num_buffers = -1);
        ...
        // Update tail pointer
        write_reg(32'h0250, num_buffers);
    endtask
    ...
    virtual task queue_tx_packet(bit [7:0] data[$], int desc_idx);
        ...
        begin
            bit [15:0] pkt_size = data.size();
            mm_agent.mem.write_byte(desc_addr + 8, pkt_size[7:0]);
            mm_agent.mem.write_byte(desc_addr + 9, pkt_size[15:8]);
        end
    ...
    virtual task wait_tx_complete(int timeout_us = 1000);
        ...
        do begin
            read_reg(32'h020C, status);  // IRQ_STATUS
            if (status & 32'h0000_0001) begin
                write_reg(32'h020C, 32'h0000_0001);  // Clear
    ...
    virtual task wait_rx_complete(int timeout_us = 1000);
        ...
        do begin
            ...
            if (status & 32'h0000_0002) begin
                write_reg(32'h020C, 32'h0000_0002);  // Clear
