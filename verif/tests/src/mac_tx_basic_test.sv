...
// Description: MAC TX path verification test
//
// Purpose:
...
class mac_tx_basic_test extends mac_base_test;
    ...
    virtual function void build_phase(uvm_phase phase);
        ...
        `uvm_info("TEST", "Building mac_tx_basic_test (true TX path verification)", UVM_LOW)
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        `uvm_info("TEST", "=== Starting mac_tx_basic_test (TX path test) ===", UVM_LOW)
        ...
        // Read count registers to verify transmitted frame count
        cnt_seq = axi_lite_read_cnt_seq::type_id::create("rd_cnt");
        cnt_seq.start(mac_sqr.axi_lite_sqr);
        
        if (cnt_seq.mac_tx_frame_cnt != num_frames) begin
            `uvm_error("TEST", $sformatf("TX frame count mismatch! Expected: %0d, Actual: %0d", 
                     num_frames, cnt_seq.mac_tx_frame_cnt))
        end else begin
            `uvm_info("TEST", $sformatf("TX frame count as expected: %0d frames transmitted.", 
                        cnt_seq.mac_tx_frame_cnt), UVM_LOW)
        end
        ...
        if (cnt_seq.mac_rx_error_cnt != 0) begin
            ...
            `uvm_info("TEST", "RX error count is zero as expected.", UVM_LOW)
        end
